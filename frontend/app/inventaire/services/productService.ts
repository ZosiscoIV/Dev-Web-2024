...
export const getProducts = async (query: string = ''):Promise<Product[]> => {
  try {
    ...
    return response.data.map(prod => 
      new Product(prod.id, prod.produit, prod.quantite, prod.prix, prod.status, prod.dateLivraison,prod.unite,prod.dateDebutVente, prod.dateFinVente, prod.categorie, prod.taxe,prod.imageURL, prod.dispo)
    );
  } catch (error: any) {
